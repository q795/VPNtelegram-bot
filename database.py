...
class Database:
    ...
    def increment_trial(self, user_id: int) -> bool:
        """Увеличить счётчик пробных периодов. Возвращает True если можно выдать ещё"""
        db = self._load()
    ...
    def use_traffic(self, user_id: int, mb: float) -> bool:
        """Использовать трафик. Возвращает True если хватило"""
        db = self._load()
    ...
    def _get_tariff_traffic(self, tariff_id: str) -> int:
        ...
        for t in TARIFFS:
            if t["id"] == tariff_id:
                return t["traffic_gb"] * 1024  # GB -> MB
        return 100 * 1024
